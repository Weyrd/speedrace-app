...
WGC (window/monitor)   ─┐                                                │
 OR OBS-hook injection ─┴▶ latest BGRA ─▶ paced writer ─▶ \\.\pipe\speedrace_video_<nonce>
...
`None` → in-ffmpeg `ddagrab` fallback. `ddagrab` is a _monitor-only_ net (it dies with
...
2. **A window pick is never the screen.** The racer picked a _window_, not their desktop. WGC
...
3. **The ffmpeg sidecar must exist and have a _working_ DTLS backend.** `-f whip` shipped in
...
| `capture_pipe.rs` | Shared plumbing both backends call: `new_video_pipe()` (the `\\.\pipe\speedrace_video_*` server) + `spawn_paced_writer()` (drains a fixed-size BGRA `latest` to the pipe at fps, black until the first frame). One implementation — do **not** re-duplicate it per backend.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
1. **Arm, don't wait for a frame.** `inject_and_arm` returns as soon as injection succeeds; it
   ...
   until frames arrive. Injection _failing_ (anti-cheat) blocks publish; "not rendering yet" does
...
3. **Teardown = drop keepalive only, never `EVENT_CAPTURE_STOP`.** That event is _global_ — it
...
4. **Normalize the texture format — don't assume BGRA.** The hook shares the game's backbuffer in
   ...
   locked at launch (before the game renders), which is _why_ the conversion must happen here, not
...
ultrawide sources keep their aspect. It applies to the live WHIP leg _and_ the MP4 replay,
